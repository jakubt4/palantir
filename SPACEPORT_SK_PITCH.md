...
| Ročník | Víťaz | Téma |
|---|---|---|
| 2025 (R4) | **Aerostacks** | Predikcia lavín cez remote sensing + AI |
...
3. **Mega-konstelácie = data deluge problem.** Java 21 prináša dve nezávislé osi škálovania pre Palantir: Virtual Threads pre tisíce súbežných blocking I/O operácií (per-satellite UDP downlinks, per-satellite TC listeners) a multi-core platform-thread CPU parallelism pre SGP4/SDP4 propagáciu (CPU-bound). Kombinácia oboch podporuje projektované škálovanie k ~10 000 satelitom na jednom COTS 16-core serveri — predmet empirického benchmarkingu v rámci Feature B roadmapy. Toto je priamo **killer feature B** z `FEATURES_v2.md`.
...
- **Design note — asymetrický CCSDS framing:** TM downlink je plne CCSDS 133.0-B-1 compliant (Primary Header, APID 100, IEEE 754 big-endian payload). TC uplink je zámerne minimalistický 1-byte opcode protokol cez UDP — XTCE `CommandContainer` definuje iba uint8 `OpCode` argument bez CCSDS Primary Header inheritance, takže Yamcs `StreamTcCommandReleaser` emituje presne 1 byte a Spring Boot ho číta ako `data[0]`. Full CCSDS TC Space Packet framing (Primary Header + APID 101 + Seq Flags + dedicated `CcsdsTelecommandParser` v Spring Boot) je explicitne Phase 3b roadmap item — nie bug, ale deliberate PoC scope decision pre zrýchlenie bidirekčného loop-u.
...
Prototype je **reference implementation**, nie akademický skript. Compiluje sa, behá, má baseline test suite, je dokumentovaný, a používa real-world protokol (CCSDS 133.0-B-1) a real-world open-source mission control systém (Yamcs — používaný vybranými ESA small-sat programmes, napr. SmallGEO, a viacerými commercial NewSpace operátormi). To je presne to, čo porota potrebuje vidieť v prvých 60 sekundách.
...
1. **Feature A — Predictive Orbital Shadowing (Model Residual Fault Detection).** Orekit engine beží paralelne s live telemetry — ideálny orbit state sa predpočítava, Yamcs porovnáva Δ medzi simulovanou a prijatou pozíciou, a `ParameterAlarm` trigger-uje pri prekročení threshold (typicky nadmerný drag residual, neočakávaný manéver, alebo katastrofálny orbit decay). Aplikácia štandardného digital twin residual-based fault detection pattern-u na orbit subsystém — prvý open-source GSaaS, ktorý to má priamo zadrôtované do Yamcs `ParameterAlarm` pipeline. Scope: orbit dynamics, nie attitude/power/thermal (tie sú v Epic 3-4 backlogu).
2. **Feature B — Multi-Axis Scaling.** Java 21 umožňuje kombinovať dve ortogonálne scaling stratégie: Virtual Threads pre tisíce súbežných per-satellite blocking I/O loopov (UDP sockets, REST listeners) a multi-core platform-thread paralelizmus pre CPU-bound SGP4/SDP4 propagation. Cieľ: projektované škálovanie k ~10 000 satelitom na jednom 16-core COTS serveri, verifikované empirickým benchmarkingom. Priama odpoveď na data deluge mega-konstelácií bez nutnosti horizontálneho scale-out-u v prvej fáze.
3. **Feature C — Closed-Loop Command Verification.** Bidirekčný ACK handshake: command NIE JE v MDB označený ako `COMPLETED`, kým nepríde potvrdzujúci telemetry paket z OBC, validovaný cez XTCE `ManualVerifier` + `TransmissionConstraint` gate. Eliminuje „blind commanding" — povinný pattern pre ESA inštitucionálny provoz a pre safety-critical COLA operations. Ako budúci follow-up (long-term backlog) plánovaná CCSDS SDLS HMAC-SHA256 autentifikácia TC paketov pre full cyber-resilience.
...
- **ESA BIC startup runway** — ~50 000 € direct financial support (grant alebo zero-interest loan podľa konkrétneho BIC) + significant in-kind services (technical support, facility access, mentoring, IPR advice). Exact podmienky a čísla pre ESA BIC Slovakia TBD v Fáze 2 programu — nemiešať direct financing s in-kind valuáciou pri investor pitchi.
...
> **Palantir** je open-source Ground Segment as a Service Digital Twin — funkčný prototype, ktorý prepája Orekit 12.2 astrodynamickú simuláciu (SGP4/SDP4) s Yamcs mission control cez CCSDS 133.0-B-1 Space Packet protokol, spúšťa sa jedným `docker compose up` a už dnes poskytuje hot-swap TLE propagation cez REST API, CCSDS telemetry downlink s XTCE decoding do Yamcs parametrov, a bidirekčný TM/TC loop. Cieľom je vytvoriť škálovateľnú cloud-native alternatívu k drahým legacy mission control systémom pre NewSpace operátorov small-sat konstelácií — roadmap pre 6-mesačný Spaceport_SK programme zahŕňa tri killer features: predictive orbital fault detection cez model residual analysis, horizontálne škálovanie pre mega-konstelácie kombináciou Java 21 Virtual Threads (concurrent per-satellite I/O) a multi-core platform-thread parallelism (SGP4 CPU compute), a closed-loop command ACK verification cez XTCE `ManualVerifier` safety gate. Konkrétny Demo Day deliverable (29. 9. 2026) je end-to-end Automated Collision Avoidance flow: PAL-501 HPC conjunction assessment proti CelesTrak debris catalogu, PAL-502 `FIRE_THRUSTER` commanding s mandatory operator approval, a Phase 3b physics reaction, kde operator na Yamcs UI vidí viditeľnú zmenu orbity po kliknutí APPROVE.

### Čo do „zloženie tímu"
- **Jakub Toth** — solo founder a core engineer. Skúsenosť s Java 21, Spring Boot, distributed systems, CCSDS, Orekit. Sprint plán pre Demo Day target (PAL-501 + PAL-502 + Phase 3b physics reaction) je **viable solo** s descope stratégiou definovanou v sekcii 5; student involvement zrýchľuje timeline a prehlbuje scope, ale nie je blocker.
- **Plánovaný expansion** — aktívne recruitujem student contributors z **UMB Banská Bystrica** (projekt prezentovaný začiatkom apríla 2026, čaká sa na commitment) pre pair-programming na PAL-501 (Epic 5 — HPC Conjunction Assessment). Demo target platí bez ohľadu na výsledok UMB response.
- **Advisory** — hľadám cez program mentorov so space sektora backgroundom (ideálne s Yamcs operation experience alebo ESA BIC relevanciou).
...
5. **August holiday gap.** Sprint 5 (August) pripadá na tradične dovolenkový mesiac v SR/EÚ. Ak sa UMB študenti zapoja, časť ich kapacity bude nedostupná; aj sám seba treba explicitne brániť proti vynechaniu toho mesiaca. Mitigation: pull-in kritických úloh zo Sprintu 5 do júla (Sprint 4), hard-deadline Phase 3b wiring najneskôr 25. 8.
6. **Sprint 6 (September) nemá reálny buffer.** Demo Day je 29. 9., Advisory Board feedback tiež v septembri. Ak Sprint 5 mešká, september nie je „buffer fáza" — je to jediná integration + rehearsal window. Mitigation: Critical Gate review 1. 9. (end-to-end demo na čistom `docker compose up` z `master` brancha); ak gate padne, aktivovať emergency descope plan.
