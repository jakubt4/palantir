...
- **`service/CcsdsTelemetrySender`** — `@Service` that encodes geodetic telemetry as 18-byte CCSDS Space Packets (6B header + 12B payload) and transmits via `DatagramSocket` over UDP. Uses `AtomicInteger` for 14-bit CCSDS sequence counter. Target host/port configurable via `@Value` with env var overrides. `@PostConstruct` creates the socket, `@PreDestroy` closes it.
...
- **`yamcs/`** — Custom Yamcs Docker image based on `yamcs/example-simulation:5.12.2`. Instance named `palantir`. Config files:
  ...
  - `etc/yamcs.palantir.yaml` — Instance config (UdpTmDataLink on port 10000, UdpTcDataLink sending to `palantir-core:10001`, GenericPacketPreprocessor with local generation time and no error detection, XTCE MDB loader, archive services, stream config with `tm_realtime` and `tc_realtime` mapped to `realtime` processor).
  ...
  - `mdb/palantir.xml` — XTCE defining CCSDS binary containers: abstract `CCSDS_Packet_Base` (6B header) with `Palantir_Nav_Packet` (APID=100 restriction, 3 IEEE 754 float parameters: Latitude deg, Longitude deg, Altitude km). CommandMetaData defines PING (opcode 0x01) and REBOOT_OBC (opcode 0x02) telecommands with uint8 opcode arguments.
- **`docker-compose.yaml`** (project root) — Orchestrates `yamcs` and `palantir-core` services. Yamcs has a healthcheck (`GET /api/`), persistent volume (`palantir_yamcs_data`), and the Spring Boot service depends on Yamcs health. `YAMCS_UDP_HOST=yamcs` env var enables Docker DNS resolution. Exposes UDP 10001 for telecommand uplink.
