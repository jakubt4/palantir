...
30. **Astrodynamic Covariance:** When simulating position uncertainty, never inject isotropic (spherical) noise. Orbital uncertainty MUST be modelled in the **Local Orbital Frame**, defined here as **RTN**: R = radial (geocentre → spacecraft), T = in-plane perpendicular to R in direction of motion, N = R × T (orbit-normal). This matches CCSDS 508.0-B-1 Conjunction Data Message convention and NASA CARA / ESA CRASS / 18th Space Defense Squadron practice. For tracked LEO objects, the canonical 1σ ratio is **R : C : T ≈ 1 : 10 : 100** (radial ~10–100 m, cross-track ~50–500 m, in-track ~100 m–several km); deviate only with a specific reason. Covariance is a **frame-bound** quantity — a 6×6 covariance in RTN cannot be used in ECI without the rotation `Σ_ECI = R · Σ_RTN · Rᵀ` at the *covariance epoch*. Mixing frames silently gives axis-misaligned noise that is harder to detect than pure-isotropic noise.
