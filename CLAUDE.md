...
26. **Time Domain Determinism:** Never use `System.currentTimeMillis()`, `Instant.now()`, or `new Date()` for physics, scheduling, or orbital logic. Time must strictly flow through Orekit's `AbsoluteDate` using the configured TimeScale (usually UTC via `TimeScalesFactory.getUTC()`). Ground Segment reception time must NEVER be used as spacecraft generation time.
27. **Deterministic Randomness (Monte Carlo):** Any random number generation (e.g., for collision assessment, Pc computation, or synthetic telemetry noise) MUST use a seeded PRNG from a math library (e.g., `org.hipparchus.random.Well19937a`). Never use `Math.random()` or unseeded instances. Execution must be 100% reproducible for regulatory audits.
28. **Sequence Integrity over Async:** In telemetry downlink and CCSDS encoding, strict chronological ordering and sequence counter monotonicity are more important than asynchronous I/O isolation. Do not introduce concurrency (like Virtual Threads) into packet serialization/transmission if it risks out-of-order UDP datagrams.
29. **Frozen Toolchain:** Do NOT suggest or execute upgrades to library versions (`pom.xml`, `pyproject.toml`, Docker images) unless explicitly instructed. Space systems rely on validated baselines; any minor version bump risks floating-point drift in the physics engine.
