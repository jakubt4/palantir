...
- **`yamcs/`** — Custom Yamcs Docker image based on `yamcs/example-simulation:5.12.2`. Instance named `palantir`. Config files:
  ...
  - `mdb/baseline.xml` — root `<SpaceSystem name="Palantir">`: CCSDS Primary Header primitives (`uint16_t`, `ccsds_packet_id`/`ccsds_seq_count`/`ccsds_length`, abstract `CCSDS_Packet_Base` container) plus nav telemetry (`Palantir_Nav_Packet` APID=100, Latitude/Longitude/Altitude as IEEE 754 float32). Yamcs paths: `/Palantir/Latitude`, `/Palantir/Longitude`, `/Palantir/Altitude`.
