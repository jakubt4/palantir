...
| Test coverage | JUnit 5 + Mockito + AssertJ — `TleIngestionControllerTest` (`@WebMvcTest`), `OrbitPropagationServiceTest` (`@SpringBootTest` + `ArgumentCaptor<Float>` physical-bounds assertions), `PalantirApplicationTests` (context load) | `mvn test` green; JaCoCo report at `palantir-core/target/site/jacoco/index.html` |
...
- **Technical contract.**
  ...
  - Test profile (`palantir-core/src/test/resources/application.yaml`) sets `enabled: false` so unit / integration tests do not hit CelesTrak in CI.
